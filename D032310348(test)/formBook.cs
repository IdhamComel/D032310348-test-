...
namespace D032310348_test_
{
    public partial class formBook : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (ValidateInputs())
            {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@PublishDate", publishDateDateTimePicker.Value);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                string query = "DELETE FROM Book WHERE ISBN-13 = @ISBN-13";
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@ISBN-13", textBoxISBN.Text);
            ...
            {
                MessageBox.Show("Please enter a AuthorID to delete.");
            }
        }
        ...
        private bool ValidateInputs()
        {
            if (string.IsNullOrEmpty(textBoxISBN.Text) || string.IsNullOrEmpty(textBoxTitle.Text) ||
                string.IsNullOrEmpty(textBoxPublisher.Text) || publishDateDateTimePicker.Value == DateTimePicker.MinimumDateTime)
            {
